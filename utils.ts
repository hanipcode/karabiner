...
interface TabResetObject {
  key: KeyCode;
  manipulator: Manipulator;
}

export function resetTabOnKeys(keys: KeyCode[]): any {
  return keys
    .map(
      (key: KeyCode) =>
        ({
    ...
    .reduce((prev, next: TabResetObject) => {
      prev[next.key] = next.manipulator;
      return prev;
    }, {} as Record<KeyCode, Manipulator>);
}
...
export function createTabSubLayer(
  ...
  return [
    ...
    {
      description: `Toggle Tab sublayer ${sublayer_key}`,
      type: "basic",
      from: {
 ...
 * activates at a time
 */
export function createTabSubLayers(subLayers: {
...
function generateSubLayerVariableName(
  key: KeyCode,
  layer_name: string = "hyper_sublayer_"
) {
